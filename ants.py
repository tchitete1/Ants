    ...
    def add_to(self, place):
        if place.ant is None:
            ...
            if isinstance(place.ant, ContainerAnt):
                assert not isinstance(self, ContainerAnt), 'Two ants in {0}'.format(place)
                assert place.ant.contained_ant is None, 'Two ants in {0}'.format(place)
                if place.ant.can_contain(self):
                    place.ant.contain_ant(self)
            # Assigns ContainerAnt to ant attribute of place first before containing self
            # Prevents removal issues
            elif isinstance(self, ContainerAnt):
                assert not isinstance(place.ant, ContainerAnt), 'Two ants in {0}'.format(place)
                existing_ant = place.ant
                place.ant = self
                assert place.ant.contained_ant is None, 'Two ants in {0}'.format(place)
                if place.ant.can_contain(existing_ant):
                    place.ant.contain_ant(existing_ant)
            else:
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, beehive):
        ...
        bee = None
        entrance_transitions = 0
        while current_place != beehive and not bee:
            bee = rANTdom_else_none(current_place.bees)
            if bee and bee.place == beehive:
                current_place = None 
            elif entrance_transitions < self.min_range or entrance_transitions > self.max_range:
                bee = None
                current_place = current_place.entrance
                entrance_transitions += 1
            else:
                current_place = current_place.entrance
                entrance_transitions += 1
...
class FireAnt(Ant):
    ...
    def reduce_armor(self, amount):
        ...
        for b in bees[:]:
            Insect.reduce_armor(b, amount + additional_damage)
...
class NinjaAnt(Ant):
    ...
    def action(self, gamestate):
        ...
        bees = self.place.bees
        for b in bees[:]:
            Ant.reduce_armor(b, self.damage)
        # END Problem 7

# BEGIN Problem 8
class WallAnt(Ant):
    """WallAnt provides protection to home base."""
...
class ContainerAnt(Ant):
    ...
    def action(self, gamestate):
        # BEGIN Problem 9 
        if self.place and self.contained_ant:
...
class ScubaThrower(ThrowerAnt):
    """A ThrowerAnt that is watersafe."""
...
# END Problem 13
    ...
    # BEGIN Problem 13
    implemented = True   # Change to True to view in the GUI
    existing_queen = False
    # END Problem 13

    def __init__(self, armor=1):
        # BEGIN Problem 13
        ScubaThrower.__init__(self, armor)
        if QueenAnt.existing_queen:
            self.is_imposter = True
        else:
            self.is_imposter = False
            QueenAnt.existing_queen = True
    ...
    def action(self, gamestate):
        ...
        """
        # BEGIN Problem 13
        if isinstance(self, QueenAnt) and self.is_imposter:
            Ant.reduce_armor(self, self.armor)
        else:
            ThrowerAnt.action(self, gamestate) # Attacks nearest bee
        
            # Doubles damage of all ants behind (if damage not already doubled)
            current_place = self.place.exit
            while current_place:
                ant = current_place.ant
                if ant:
                    if isinstance(ant, QueenAnt) and not QueenAnt.i_real_queen:
                        Ant.reduce_armor(ant, ant.armor)
                    elif isinstance(ant, ContainerAnt):
                        if not ant.is_double_damage:
                            ant.damage *= 2
                            ant.is_double_damage = True
                        if ant.contained_ant and not ant.contained_ant.is_double_damage:
                            ant.contained_ant.damage *= 2
                            ant.contained_ant.is_double_damage = True
                    else:
                        if not ant.is_double_damage:
                            ant.damage *= 2
                            ant.is_double_damage = True
    ...
    def reduce_armor(self, amount):
        ...
        if not self.is_imposter and self.armor <= 0:
            bees_win()
        # END Problem 13
